...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<import("next").Metadata> {
  ...
  const essay = await getEssayMetadata(Number(id)).catch((error) => {
    console.error("Failed to fetch essay metadata:", error);
    return null;
  });
  ...
  return {
    title: essay.title || "Sin Título",
    description: essay.subtitle || essay.observations || "Ensayo histórico",
    openGraph: {
      title: essay.title || "Sin Título",
      description: essay.subtitle || essay.observations || "Ensayo histórico",
      type: "article",
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-zinc-950 pb-20">
      <div className="relative h-[40vh] w-full bg-zinc-900 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1455390582262-044cdead277a?q=80&w=2666&auto=format&fit=crop')] bg-cover bg-center brightness-50" />
        <Navbar />
        <div className="relative z-20 container mx-auto h-full flex items-end pb-12 px-4">
          <div className="max-w-4xl">
            <div className="mb-4 flex items-center gap-3">
              <span className="bg-emerald-600/90 text-white px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-sm">Ensayo</span>
            </div>
            <h1 className="text-3xl md:text-5xl font-bold text-white tracking-tight leading-tight">{essay.title || "Sin Título"}</h1>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 max-w-3xl -mt-10 relative z-30">
        <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-xl p-8 md:p-12 border border-gray-100 dark:border-zinc-800">
          ...
          <div className="mt-12 pt-8 border-t border-gray-100 dark:border-zinc-800 flex justify-between items-center text-sm text-gray-500">
            <span>Ref: {essay.id}</span>
            {essay.observations && <span>{essay.observations}</span>}
          </div>
        </div>
      </div>
    </main>
