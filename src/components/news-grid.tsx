...
export function NewsGrid({ news }: { news: (typeof articles.$inferSelect)[] }) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
            {news.map((item) => (
                <Link key={item.id} href={`/article/${item.id}`} className="bg-white dark:bg-zinc-900 rounded-xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 dark:border-zinc-800 flex flex-col h-full group block">
