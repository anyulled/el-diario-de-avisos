...
async function testContentSearch() {
    ...
    SELECT 
      ...
      ts_rank(search_vector, to_tsquery('spanish', 'gobierno')) as rank
    FROM articulos
    WHERE search_vector @@ to_tsquery('spanish', 'gobierno')
    ORDER BY rank DESC
    LIMIT 5
  `);
    ...
    SELECT 
      arti_titulo as title,
      ts_rank(search_vector, to_tsquery('spanish', 'teatro')) as rank
    FROM articulos
    WHERE search_vector @@ to_tsquery('spanish', 'teatro')
    ORDER BY rank DESC
    LIMIT 5
  `);
