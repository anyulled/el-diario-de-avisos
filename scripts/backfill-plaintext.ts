...
async function main() {
  ...
  // Count
  const [{ value: total }] = await db.select({ value: count() }).from(articles).where(isNull(articles.plainText));
  ...
  while (true) {
    const batch = await db.select({ id: articles.id, content: articles.content }).from(articles).where(isNull(articles.plainText)).limit(BATCH_SIZE);
    ...
    await Promise.all(
      batch.map(async (item) => {
        try {
          ...
          await db.update(articles).set({ plainText: text }).where(eq(articles.id, item.id));
        } catch (err) {
